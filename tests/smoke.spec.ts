...
test('projectenoverzicht start leeg en maakt een nieuw project aan', async ({ page }) => {
  ...
  // Vóór de schrijfagent is gestart toont het veld alleen een samenvatting, geen concept.
  await expect(page.getByText('Samenvatting aanbesteding')).toBeVisible()
  await expect(page.getByText('Nog geen concept geschreven')).toBeVisible()
  await expect(page.getByText('Brons versie')).toHaveCount(0)
...
test('genereert concept en voert AI-review uit', async ({ page }) => {
  ...
  // Na het schrijven is de startsamenvatting verdwenen en heet de knop weer "Genereer".
  await expect(page.getByText('Nog geen concept geschreven')).toHaveCount(0)
...
test('verwerkt menselijke opmerkingen via AI', async ({ page }) => {
  await createProject(page)
  // Opmerkingen verwerk je op een geschreven concept; start daarom eerst de schrijfagent.
  await page.getByRole('button', { name: 'Start schrijfagent' }).first().click()
  await expect(page.getByText('Brons versie')).toBeVisible({ timeout: 15000 })
